...
describe('box nodes', function() {
    ...
    describe("box credentials", function() {
        ...
        it("should complete oauth dance", function(done) {
            helper.load(boxNode, [{
                ...
                helper.request()
                    .get(
                        '/box-credentials/auth?id=box-config&clientId=CLIENT&clientSecret=SECRET&callback=http://localhost:1880/box-credentials/auth/callback'
                    )
                    .expect(302)
                    .expect('Location',
    ...
    describe("watch node", function() {
        ...
        it('should report an event', function(done) {
            nock('https://api.box.com')
                ...
                nock('http://2.realtime.services.box.net')
                    .get('/subscribe')
                    .query({
                        channel: 'cc807c9c4869ffb1c81a',
                        stream_position: 1000,
                        stream_type: 'all'
                    })
                    .reply(200, {message: 'new_change'});
            helper.load(boxNode,
                    ...
                    output.on("input", function(msg) {
                        try {
                            msg.payload.should.deepEqual({
                                event_id: '1234',
                                event_type: 'ITEM_UPLOAD',
                                fullPath: 'node-red/foobar.txt',
                                session_id: '1234567',
                                source: {
                                    id: '7',
                                    name: 'foobar.txt',
                                    parent:  { id: '2', name: 'node-red', type: 'folder' },
                                    path_collection: {
                                      entries: [
                                         { id: '0', name: 'All Files', type: 'folder' },
                                         { id: '2', name: 'node-red', type: 'folder' }
                                      ],
                                      total_count: 2
                                    },
                                    type: 'file'
    ...
    describe('out node', function() {
        ...
        it('should overwrite file w/ msg.payload', function(done) {
            nock('https://api.box.com')
                .get('/2.0/folders/0/items')
                .reply(200, {
                    "entries": [{
                ...
                .options('/2.0/files/3/content')
                .reply(200);
            nock('https://upload.box.com')
                .post('/api/2.0/files/3/content')
                .reply(200, {
                    "total_count": 1,
                    "entries": [{
                        "type": "file",
            ...
            helper.load(boxNode, [{
                ...
                output.should.have.property('id', 'output');
                output.on("input", function(msg) {
                    msg.payload.entries[0].should.have.property('name', 'foobar.txt');
                    done();
                });
                inject.send({
                    filename: 'node-red/foobar.txt',
        ...
        it('should upload new file w/ msg.payload', function(done) {
            nock('https://api.box.com')
                .get('/2.0/folders/0/items')
                .times(2)
                .reply(200, {
