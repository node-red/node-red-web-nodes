        ...
        this.on('input', function(msg){
            ...
            function peopleSearchRequest(cb){
                ...
                if(!query){
                    var error = {
                        ...
                        message: RED._("plus.errors.no-query")
                    };
            ...
            function peopleListRequest(cb){
                if(!collection){
                    var error = {
                        code: 400,
                        status: 'MISSING_VALUES',
                        message: RED._("plus.errors.no-collection")
                    };
                    throwNodeError(error, msg);
                    return;
                }
            ...
            function activitiesIdRequest(cb){
                if(!activityId){
                    var error = {
                        code: 400,
                        status: 'MISSING_VALUES',
                        message: RED._("plus.errors.no-activityid")
                    };
                    throwNodeError(error, msg);
                    return;
                }
            ...
            function commentsIdRequest(cb){
                if(!commentId){
                    var error = {
                        code: 400,
                        status: 'MISSING_VALUES',
                        message: RED._("plus.errors.no-commentid")
                    };
                    throwNodeError(error, msg);
                    return;
                }
            ...
            function handleCommentsIdRequest(data, cb){
                if(data.hasOwnProperty('error')){
                    ...
                    var newMsg = cloneMsg(msg);     //quick clone msg
                    newMsg.data = data;
                    newMsg.title = RED._("plus.messages.comment") + " " + data.id;
                    newMsg.payload = {
                        id: data.id,
                        actor: {
                            id: data.actor.id,
            ...
            if(node.google && node.google.credentials){
                ...
                */  accessToken = msg.credentials.accessToken;
                ...
                throwNodeError({
                    code: 400,
                    status: 'MISSING_VALUES',
                    message: RED._("plus.errors.no-accesstoken")
                }, msg);
                return;
            }
        });
